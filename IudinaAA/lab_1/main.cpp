...
public:
	...
	list(unsigned size); // list with 'size' default values
	list(unsigned size, const T& val); // list with 'size' setted values

	unsigned size() const; // size of a list
	void resize(unsigned size); // resizes list witn new size
	bool insert(unsigned pos, const T& val); // inserts value in setted position
	void push_back(const T& val); // ads element in tne end pf list
	bool contains(const T& val); // checks if list contains that element
	T& operator [] (unsigned index); // finds element with setted index
	list& operator=(const list& clist); // assignment operator
	int find(const T& val); // finds index for setted element
